...
if (isS2) {
  ...
  var activeList = (ev.active || []).map(function(k) {
    var info = evInfo(k);
...
var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>任务板 - ' + state.season + '</title><style>'
  ...
  + (isS2 && state.events && state.events.active && state.events.active.length ? ' · 🌬 ' + state.events.active.map(function(k){ return evInfo(k).name; }).join('、') : '') + '</div>'
