...
// ===== 排名分 =====
function rankScore(rank) { return -11 * Math.log(rank) / Math.log(12) + 12; }
...
function buildCtx(q, qi, allQ, allStats, allAnswers, score, sorted, rankAnswers, pass, myAnswers, myName) {
  ...
  for (let i = 0; i < allQ.length; i++) {
    const aq = allQ[i], at = aq.type || 'single', aa = allAnswers[i];
    const qkey = 'q' + (i + 1);
  ...
  if (type === 'single') {
    ...
    ctx.ladder_score = computeLadder(qi, myVal, allAnswers.map(a => a.value || ''));
  }
  return ctx;
}
...
function scoreAllQuestions(questions, rawAnswers) {
  ...
  for (const r of rawAnswers) {
    ...
    for (let qi = 0; qi < questions.length; qi++) {
      const q = questions[qi];
      const ctx = buildCtx(q, qi, allQ, allStats, r.answers, sc, null, null, 0, r.answers, r.name);
  ...
  for (const r of rawAnswers) {
    ...
    for (let qi = 0; qi < questions.length; qi++) {
      ...
      var ctx = buildCtx(q, qi, allQ, allStats, r.answers, sc, sorted1, rankAns1, 1, r.answers, r.name);
...
const TASK_DEFS = [
  ...
  { id: 'A04', list: 'A', desc: '总分达到{x}分', paramStyle: 'normal', paramMean: 100, paramStddev: 33 },
  { id: 'A05', list: 'A', desc: '总排名分达到{x}', paramStyle: 'normal', paramMean: 50, paramStddev: 16 },
  { id: 'A06', list: 'A', desc: '连续{x}次排名1~6', paramStyle: 'weighted', paramWeights: [6, 3, 1], paramVals: [2, 3, 4] },
  { id: 'A07', list: 'A', desc: '连续{x}次在T1选择得分最高的选项', paramStyle: 'weighted', paramWeights: [6, 3, 1], paramVals: [2, 3, 4] },
  { id: 'A08', list: 'A', desc: '总计{x}次在T1选择得分最高的选项', paramStyle: 'weighted', paramWeights: [3, 4, 6, 3, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  { id: 'A09', list: 'A', desc: '连续{x}次在T2选择得分最高的选项', paramStyle: 'weighted', paramWeights: [6, 3, 1], paramVals: [2, 3, 4] },
  { id: 'A10', list: 'A', desc: '总计{x}次在T2选择得分最高的选项', paramStyle: 'weighted', paramWeights: [3, 4, 6, 3, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  { id: 'A11', list: 'A', desc: '连续{x}次在T3选择得分最高的选项', paramStyle: 'weighted', paramWeights: [6, 3, 1], paramVals: [2, 3, 4] },
  { id: 'A12', list: 'A', desc: '总计{x}次在T3选择得分最高的选项', paramStyle: 'weighted', paramWeights: [3, 4, 6, 3, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  { id: 'A13', list: 'A', desc: '连续{x}次在T4选择得分最高的选项', paramStyle: 'weighted', paramWeights: [6, 3, 1], paramVals: [2, 3, 4] },
  { id: 'A14', list: 'A', desc: '总计{x}次在T4选择得分最高的选项', paramStyle: 'weighted', paramWeights: [3, 4, 6, 3, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  { id: 'A15', list: 'A', desc: '连续{x}次在T5选择得分最高的选项', paramStyle: 'weighted', paramWeights: [6, 3, 1], paramVals: [2, 3, 4] },
  { id: 'A16', list: 'A', desc: '总计{x}次在T5选择得分最高的选项', paramStyle: 'weighted', paramWeights: [3, 4, 6, 3, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  ...
  { id: 'A25', list: 'A', desc: '总计{x}次在T1选择得分最低的选项', paramStyle: 'weighted', paramWeights: [6, 4, 4, 2, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  { id: 'A26', list: 'A', desc: '总计{x}次在T2选择得分最低的选项', paramStyle: 'weighted', paramWeights: [6, 4, 4, 2, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  { id: 'A27', list: 'A', desc: '总计{x}次在T3选择得分最低的选项', paramStyle: 'weighted', paramWeights: [6, 4, 4, 2, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  { id: 'A28', list: 'A', desc: '总计{x}次在T4选择得分最低的选项', paramStyle: 'weighted', paramWeights: [6, 4, 4, 2, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  { id: 'A29', list: 'A', desc: '总计{x}次在T5选择得分最低的选项', paramStyle: 'weighted', paramWeights: [6, 4, 4, 2, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  ...
  { id: 'A34', list: 'A', desc: '获得{x}次排名5~8', paramStyle: 'weighted', paramWeights: [6, 4, 4, 2, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  { id: 'A35', list: 'A', desc: '获得{x}次排名9~12', paramStyle: 'weighted', paramWeights: [6, 4, 4, 2, 2, 1, 1], paramVals: [1, 2, 3, 4, 5, 6, 7] },
  ...
  { id: 'B03', list: 'B', desc: '单回合在T2~T5中，全部选中得分最低项之一', paramStyle: 'none' },
  { id: 'B04', list: 'B', desc: '单回合在T2~T5中，全部选中得分最高项之一', paramStyle: 'none' },
  { id: 'B05', list: 'B', desc: '单回合在T2~T5中，全部未选中得分最低或最高项之一', paramStyle: 'none' },
  ...
  { id: 'B12', list: 'B', desc: '累计获得{x}次单题最低分', paramStyle: 'normal', paramMean: 15, paramStddev: 4 },
  { id: 'B13', list: 'B', desc: '某回合你的作答与另一玩家至少完全重合{x}题', paramStyle: 'weighted', paramWeights: [4, 3, 2, 1], paramVals: [4, 5, 6, 7] },
  { id: 'B14', list: 'B', desc: '在T6回答过所有一位数答案', paramStyle: 'none' },
  ...
  { id: 'B17', list: 'B', desc: '连续3次在除T10外某题获得最高分', paramStyle: 'none' },
  { id: 'B18', list: 'B', desc: '连续3次在除T10外某题获得最低分', paramStyle: 'none' },
  ...
  { id: 'B23', list: 'B', desc: '在某题拿到最多分数，且比次多的玩家至少多获得5分', paramStyle: 'none' },
  { id: 'B24', list: 'B', desc: '单回合所有题目均未获得恰好0分', paramStyle: 'none' },
  { id: 'B25', list: 'B', desc: '某回合你未完成你的盘面上的任何未完成任务', paramStyle: 'none' },
  { id: 'B26', list: 'B', desc: '你的问卷填写时间在所有人中位列前三', paramStyle: 'none' },
